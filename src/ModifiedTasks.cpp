    ...
    namespace lipm_stabilizer
    {
        ...
        void StabilizerTask_Zyc::run()
        {
            ...
            else
            {
                ...
                fSumFilter_.update(Eigen::Vector3d{0, 0, robot().mass() * mc_rtc::constants::GRAVITY});
            }
        ...
        void StabilizerTask_Zyc::checkInTheAir()
        {
            inTheAir_ = true;
            for (const auto &footT : footTasks)
            {
            ...
            if (!wasInTheAir_ && inTheAir_)
            {
                ...
                if (enabled_)
                {
                    mc_rtc::log::warning("[{}] Robot is in the air, disabling stabilizer ZYC", name());
                    disable_();
                }
            }
            else if (!inTheAir_ && wasInTheAir_)
            {
                ...
                if (wasEnabled_)
                {
                    mc_rtc::log::warning("[{}] Robot is no longer in the air, re-enabling stabilizer ZYC", name());
                    enable();
                }
            }
        }

        void StabilizerTask_Zyc::computeLeftFootRatio()
        {
            if (inDoubleSupport())
            {
        ...
        void StabilizerTask_Zyc::updateState(const Eigen::Vector3d &com,
        ...
        void StabilizerTask_Zyc::updateContacts(mc_solver::QPSolver &solver)
        {
        ...
        sva::ForceVecd StabilizerTask_Zyc::computeDesiredWrench()
        {
        ...
        void StabilizerTask_Zyc::distributeWrench(const sva::ForceVecd &desiredWrench)
        {
            // Variables
            // ---------
            // x = [w_l_0 w_r_0] where
        ...
        void StabilizerTask_Zyc::computeCoPonHorizon(const std::vector<Eigen::Vector2d> &zmp_ref,
                                                     const double delta,
                                                     const double t_delay)
        {
            ...
            measuredLeftCoP_delayed.x() = measuredLeftCoP.x() * exp(-c_.copFzLambda.x() * t_delay) + (1 - exp(-c_.copFzLambda.x() * t_delay)) * delayedTargetCoPLeft_.x();
            measuredLeftCoP_delayed.y() = measuredLeftCoP.y() * exp(-c_.copFzLambda.y() * t_delay) + (1 - exp(-c_.copFzLambda.y() * t_delay)) * delayedTargetCoPLeft_.y();

            Eigen::Vector2d measuredRightCoP_delayed;
            double measuredFzRight_delayed = 0;
            measuredRightCoP_delayed.x() = measuredRightCoP.x() * exp(-c_.copFzLambda.x() * t_delay) + (1 - exp(-c_.copFzLambda.x() * t_delay)) * delayedTargetCoPRight_.x();
            measuredRightCoP_delayed.y() = measuredRightCoP.y() * exp(-c_.copFzLambda.y() * t_delay) + (1 - exp(-c_.copFzLambda.y() * t_delay)) * delayedTargetCoPRight_.y();

            if (supportFoot_ == ContactState::Left)
            {
                measuredFzRight_delayed = measuredFzRight * exp(-c_.copFzLambda.z() * t_delay) + (1 - exp(-c_.copFzLambda.z() * t_delay)) * delayedTargetFzRight_;
                measuredFzLeft_delayed = fz_tot - measuredFzRight_delayed;
            }
            else
            {
                measuredFzLeft_delayed = measuredFzLeft * exp(-c_.copFzLambda.z() * t_delay) + (1 - exp(-c_.copFzLambda.z() * t_delay)) * delayedTargetFzLeft_;
            ...
            Eigen::Vector2d QPzmp =
                (1 - ratio0) * (X_0_lc.translation().segment(0, 2) + X_0_lc.rotation().block(0, 0, 2, 2).transpose() * QPCoPLeft_) + ratio0 * (X_0_rc.translation().segment(0, 2) + X_0_rc.rotation().block(0, 0, 2, 2).transpose() * QPCoPRight_);
        ...
        void StabilizerTask_Zyc::distributeCoPonHorizon(const std::vector<Eigen::Vector2d> &zmp_ref, const double delta)
        {
            if (zmp_ref.size() == 0)
            {
            ...
            {
                modeledCoPLeft_.x() =
                    (footTasks[ContactState::Left]->targetCoP() + (modeledCoPLeft_ - footTasks[ContactState::Left]->targetCoP()) * exp(-c_.copFzLambda.x() * dt_))
                        .x();
                modeledCoPLeft_.y() =
                    (footTasks[ContactState::Left]->targetCoP() + (modeledCoPLeft_ - footTasks[ContactState::Left]->targetCoP()) * exp(-c_.copFzLambda.y() * dt_))
                        .y();
                modeledCoPRight_.x() =
                    (footTasks[ContactState::Right]->targetCoP() + (modeledCoPRight_ - footTasks[ContactState::Right]->targetCoP()) * exp(-c_.copFzLambda.x() * dt_))
                        .x();
                modeledCoPRight_.y() =
                    (footTasks[ContactState::Right]->targetCoP() + (modeledCoPRight_ - footTasks[ContactState::Right]->targetCoP()) * exp(-c_.copFzLambda.y() * dt_))
                        .y();
                modeledFzLeft_ = (footTasks[ContactState::Left]->targetWrench().force().z() + (modeledFzLeft_ - footTasks[ContactState::Left]->targetWrench().force().z()) * exp(-c_.copFzLambda.z() * dt_));
                modeledFzRight_ = (footTasks[ContactState::Right]->targetWrench().force().z() + (modeledFzRight_ - footTasks[ContactState::Right]->targetWrench().force().z()) * exp(-c_.copFzLambda.z() * dt_));
            }
        }

        void StabilizerTask_Zyc::saturateWrench(const sva::ForceVecd &desiredWrench,
        ...
        void StabilizerTask_Zyc::setSupportFootGains()
        {
            if (inDoubleSupport())
            {
        ...
        void StabilizerTask_Zyc::updateCoMTaskZMPCC()
        {
        ...
        void StabilizerTask_Zyc::updateFootForceDifferenceControl()
        {
            ...
            Eigen::Vector3d df_ctrl =
                R_0_fb_yaw.transpose() * (gainAdmittance.cwiseProduct(R_0_fb_yaw * dfForceError_) - gainDamping.cwiseProduct(R_0_fb_yaw * dfError_));
        ...
        void StabilizerTask_Zyc::updateZMPFrame()
        {
            if (inDoubleSupport())
            {
        ...
        template <sva::ForceVecd StabilizerTask_Zyc::ExternalWrench::*TargetOrMeasured>
        void StabilizerTask_Zyc::computeWrenchOffsetAndCoefficient(const mc_rbdyn::Robot &robot,
        ...
        template <sva::ForceVecd StabilizerTask_Zyc::ExternalWrench::*TargetOrMeasured>
        sva::ForceVecd StabilizerTask_Zyc::computeExternalWrenchSum(const mc_rbdyn::Robot &robot,
        ...
        void StabilizerTask_Zyc::computeExternalContact(const mc_rbdyn::Robot &robot,
        ...
        void StabilizerTask_Zyc::update(mc_solver::QPSolver &solver)
        {
